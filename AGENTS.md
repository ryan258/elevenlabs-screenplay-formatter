...
## Testing Guidelines
There is no automated suite yet, so rely on manual checks. Use `EXAMPLE_SCREENPLAY.md` or `plays/` samples to confirm parsing, character detection, and per-character settings. Always exercise both output modes: run just `npm run dev` for individual MP3 downloads, then repeat with the backend running to ensure `/concatenate` succeeds and a single file lands in `Downloads`. When editing `utils/elevenLabsApi.ts`, monitor console logs for rate-limit handling and confirm `OutputDisplay` still streams progress.
