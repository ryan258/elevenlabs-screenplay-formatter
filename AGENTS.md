...
## Testing Guidelines
Use `npm run test` for parser unit tests and `npm run lint` / `npm run check` before commits. For manual QA, load `EXAMPLE_SCREENPLAY.md`, `EXAMPLE_FOUNTAIN.md`, or files in `plays/` to confirm parsing, diagnostics, and per-character settings. Always exercise both output modes: run just `npm run dev` for individual MP3 downloads, then repeat with the backend running to ensure `/concatenate` succeeds and a single file lands in `Downloads`. When editing `utils/elevenLabsApi.ts`, monitor the progress log for retry/resume behavior and ensure the Output/Diagnostics panels still stream updates.
